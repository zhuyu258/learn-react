...
function A(props){
  return (
    <div>
      ...
      <button onClick={()=>{
        props.history.push('/RouteMsg/B','b的状态')
...
function B(props){
  return (
    <div>
      ...
      <p>{JSON.stringify(props)}</p>
      <button onClick={()=>{
        props.history.push('/RouteMsg/A?a=1&b=2','A的状态')
      ...
      <>
        ...
        4. 路由信息
           ...
           4.1 history对象
               ...
               goBack() 回到上一个路径
               listen: 监听地址的变化，传递一个函数，当地址变化的时候执行这个函数，
                      函数有两个参数：
                        参数1： location对象，记录当前的地址信息
                        参数2： action,一个字符串表示跳转时的方式
        ...
        <a href="/RouteMsg/A">跳转到A页面</a><br/>
        <a href="/RouteMsg/B">跳转到B页面</a>
        <Switch>
          <Route component={A} exact path="/RouteMsg/A" >AA</Route>
          <Route component={B} exact path="/RouteMsg/B" />
        </Switch>
      </>
    );
  }
